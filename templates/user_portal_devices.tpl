...
{% wire id="arrows_"++#dtid type="click"
        ...
        action={ postback postback={trigger_innoui_widget arg="user_devices_widget_opened" } delegate="inno" }
%}
  <span id="arrows_{{ #dtid }}" style="cursor: pointer;">
    <i id="arrow_right_{{ #dtid }}" style="{% if m.inno[{ui_element_opened element="user_devices_widget_opened"}] %}display: none;{% endif %}"
                                    class="arrowpad fa fa-arrow-circle-right"></i>
    <i id="arrow_down_{{ #dtid }}" style="{% if not m.inno[{ui_element_opened element="user_devices_widget_opened"}] %}display: none;{% endif %}"
...
<div id="user_devices_widget_opened" style="{% if not m.inno[{ui_element_opened element="user_devices_widget_opened"}] %}display: none;{% endif %}">
<table id="user_portal_devices_table" class="table display table-striped table-condensed">
    ...
    <tbody>
        ...
        {% with m.inno[{kz_get_device_registration_details device_id=device["id"]}] as device_details %}
        {% with m.inno[{kz_check_device_registration device_id=device["id"]}] as device_registered %}
	<tr>
            <td style="text-align: center;">
                ...
                {% elseif device["device_type"]=="softphone" %}
                  <i class="{% if device["enabled"] %}
                                {% if m.inno[{kz_check_device_registration device_id=device["id"]}] %}registered
                                    {% else %}unregistered
                                {% endif %}
                            {% endif %} 
...
{# print m.inno.kz_list_user_devices #}
