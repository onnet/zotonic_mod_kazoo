...
<div class="row" style="margin-top: 2em; margin-bottom: 2em;">
  {% wire id="search_account_btn"
          action={dialog_open title=_"Account lookup "
                              template="_rs_search_dialog.tpl"
                 }
  %}
  <div class="col-xs-2 {% if not account_id %}col-xs-offset-2{% endif %}">
    <button id="search_account_btn"
    ...
    <select id="child_selector"
      ...
      {% for option in m.kazoo.kz_list_account_children %}
          <option value="{{ option[1]["id"] }}"
                  {% if option[1]["id"] == account_id %}selected{% endif %}>
...
{% if account_id %}
  <div class="col-xs-2">
    {% wire id="user_mask_selector"
            type="change"
            action={postback postback={rs_account_mask account_id=account_id}
                             delegate="mod_kazoo"
                   }
    %}
...
{% if account_id %}
  {% wire id="rs_account_delete_btn"
          action={confirm text="Do you really want to delete this account?"
                          action={postback postback={rs_account_delete account_id=account_id}
                                           delegate="mod_kazoo"
                                 }
                 }
  %}
  <div class="col-xs-2">
    <button id="rs_account_delete_btn"
