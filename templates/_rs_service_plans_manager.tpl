...
<table class="table table-condensed table-centered">
    ...
    <tr>
      <th colspan="2">
        {% wire id="arrows_"++#spid type="click"
                ...
                action={postback postback={trigger_innoui_widget arg="rs_service_plans_widget_opened"}
                                 delegate="mod_kazoo"
                       }
        %}
          <span id="arrows_{{ #spid }}" style="cursor: pointer; padding-left: 0.7em;">
            <i id="arrow_right_{{ #spid }}"
               style="{% if m.kazoo[{ui_element_opened element="rs_service_plans_widget_opened"}] %}
                        display: none;
                      {% endif %}" 
               class="arrowpad fa fa-arrow-circle-right"></i>
            <i id="arrow_down_{{ #spid }}"
               style="{% if not m.kazoo[{ui_element_opened element="rs_service_plans_widget_opened"}] %}
                        display: none;
                      {% endif %}" 
           ...
           {% button class="btn btn-xs btn-onnet pull-right"
                     ...
                     action={confirm text=_"Do you really want to reconcile services?"
                                     action={postback postback={reconcile_account_services account_id=account_id}
                                                      delegate="mod_kazoo"
                                            }
                            }
           %}
           {% button class="btn btn-xs btn-onnet pull-right"
                     ...
                     action={confirm text=_"Do you really want to sync services?"
                                     action={postback postback={sync_account_services account_id=account_id}
                                                      delegate="mod_kazoo"
                                            }
                            }
           %}
  ...
  <tbody id="rs_service_plans_widget_opened"
         style="border-top: 0px;
                {% if not m.kazoo[{ui_element_opened element="rs_service_plans_widget_opened"}] %}
                  display: none;
                {% endif %}">
      ...
      <td colspan="2">
       {_ Choose service plan to add _}:
        ...
        {% for service_plan_available in m.kazoo[{kz_service_plans_available account_id=account_id}] %}
          <option name="service_plan_available" value="{{ service_plan_available[1]["id"] }}">
            {{ service_plan_available[1]["name"] }}
          </option>
        {% endfor %}
        </select>
       ...
       <span class="pull-right pl-15">
         <i id="close_add_service_plan" class="fa fa-times pointer"></i>
       </span>
    ...
    {% for service_plan in m.kazoo[{kz_current_service_plans account_id=account_id}][1]["plans"][1] %}
      {% with service_plan[1],
              m.kazoo[{service_plan plan_id=service_plan[1]}]
           as
              service_plan_id,
              service_plan_doc
      %}
        ...
        <td colspan="2">
          ...
          {% wire id="delete_"++service_plan_id
               action={confirm text=_"Do you really want to remove "++service_plan_id++" service plan?"
                               action={postback postback={remove_service_plan_from_account
                                                          account_id service_plan_id
                                                         }
                                                inject_args
          ...
          %}
          <i id="delete_{{ service_plan_id }}"
             class="fa fa-trash-o pointer pull-right pr-1"
             title="{_ Delete _}"></i>
        </td>
      </tr>
      {% endwith %}
