<div id="{{ element_id }}_child" class="child">
  ...
  {% endif %}
  {% droppable id=element_id
               tag={drop_args drop_id=element_id drop_parent=tool_name}
               delegate="mod_kazoo"
  %}
  <div id="{{ element_id }}" class="branch">
    <div name="{{ tool_name }}" class="node icons_black" style="position: relative;">
      <div class="node-options">
        {% wire id=element_id++"_delete"
                action={remove target=element_id++"_child"}
                action={postback postback={cf_delete_element element_id=element_id}
                                 delegate="mod_kazoo"
                       }
        %}
        <div id="{{ element_id }}_delete" class="delete"></div>
        {% wire id=element_id++"_cf_park"
                action={postback postback={cf_park_element element_id=element_id}
                                 delegate="mod_kazoo"
                       }
        %}
        <div id="{{ element_id }}_cf_park" class="cf_park"></div>
        <div class="module">
          {% wire id=element_id++"_div_icon"
                  action={dialog_open title=_"Edit"++" "
                                      template="_cf_select_"++tool_name++".tpl"
    ...
    <div id="{{ element_id }}-children" class="children">
      {% wire action={postback postback={check_children id=element_id
                                                        drop_id=element_id
                                                        drop_parent=tool_name
                                        }
                               delegate="mod_kazoo"
                     }
      %}
