...
<div class="row" style="margin-top: 2em; margin-bottom: 1em;">
  ...
  {% endjavascript %}
  <div class="col-xs-2">
    <select id="account_list_selector"
            name="selected_list"
            class="col-xs-12 form-control margin-bottom-xs selectpicker"
            title="{_ Participants list _}"
            data-live-search="true">
      {% for list in m.kazoo.kz_list_account_lists %}
        <option value="{{ list[1]["id"] }}">{{ list[1]["name"] }}</option>
      {% endfor %}
    </select>
  </div>
  {% javascript %}
  ...
  {% endjavascript %}
  <div class="col-xs-2">
    <select id="media_selector"
            name="selected_media"
            class="col-xs-12 form-control margin-bottom-xs selectpicker"
            style="text-align:center;display: none;"
            title=" -- {_ Play media _} -- "
            data-live-search="true">
  ...
  {% endjavascript %}
  {% wire id="start_conference_btn"
          action={confirm text="Do you really want to start conference?"
  ...
  %}
  <div class="col-xs-2">
    <button id="start_conference_btn"
