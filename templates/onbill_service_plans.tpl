...
{% wire id="arrows_"++#dtid type="click"
        ...
        action={postback postback={trigger_innoui_widget arg="onbill_service_plan_widget_opened"}
                         delegate="mod_kazoo"
               }
%}
  <span id="arrows_{{ #dtid }}" style="cursor: pointer;">
    <i id="arrow_right_{{ #dtid }}"
       style="{% if m.kazoo[{ui_element_opened element="onbill_service_plan_widget_opened"}] %}
                display: none;
              {% endif %}"
       class="arrowpad fa fa-arrow-circle-right"></i>
    <i id="arrow_down_{{ #dtid }}"
       style="{% if not m.kazoo[{ui_element_opened element="onbill_service_plan_widget_opened"}] %}
                display: none;
              {% endif %}"
    ...
    {% button class="btn btn-xs btn-onnet pull-right"
              text=_"create"
              id="add_json_service_plan"
              action={dialog_open title=_"Create new service plan."
                                  template="_create_new_service_plan.tpl"
                     }
     %}
    ...
    <div class="btn-group pull-right" style="margin-left: 3px;">
      ...
      <ul class="dropdown-menu nav-list nav">
        {% wire name="service_plan_edit_event"
                action={update target="onbill_service_plan_widget_opened"
        ...
        {% for service_plan in m.kazoo.kz_service_plans %}
          <li>
            <a href="#"
               onclick="z_event('service_plan_edit_event', { service_plan_id: '{{ service_plan[1]["id"] }}' });">
                {{ service_plan[1]["name"] }}
            </a>
          </li>
        {% endfor %}
...
<div id="onbill_service_plan_widget_opened"
     style="{% if not m.kazoo[{ui_element_opened element="onbill_service_plan_widget_opened"}] %}
              display: none;
            {% endif %}">
</div>
