...
{% wire id="arrows_"++#dtid type="click"
        ...
        action={ postback postback={trigger_innoui_widget arg="user_settings_widget_opened" } delegate="inno" }
%}
  <span id="arrows_{{ #dtid }}" style="cursor: pointer;">
    <i id="arrow_right_{{ #dtid }}" style="{% if m.inno[{ui_element_opened element="user_settings_widget_opened"}] %}display: none;{% endif %}" 
                                    class="arrowpad fa fa-arrow-circle-right"></i>
    <i id="arrow_down_{{ #dtid }}" style="{% if not m.inno[{ui_element_opened element="user_settings_widget_opened"}] %}display: none;{% endif %}"
                                   class="arrowpad fa fa-arrow-circle-down"></i>
  </span>
    {{ headline }}
    {% if not m.inno[{ui_element_opened element="user_settings_widget_opened"}] %}
...
{% wire id="user_settings_form_form" type="submit" postback={kazoo_user_settings} delegate="inno" %}

<div id="user_settings_widget_opened" style="{% if not m.inno[{ui_element_opened element="user_settings_widget_opened"}] %}display: none;{% endif %}">
<form  id="user_settings_form_form" method="post" action="postback">
    <div style="padding-top: 1.5em; padding-left: 1em;">
      {% with m.inno.get_user_timezone as user_timezone %}
    ...
    <div class="form-inline" style="padding: 1em;">
        <label>Enable Call Forward:</label>
            {% if m.inno[{kz_user_doc_field field1="call_forward" field2="enabled"}] %}
    ...
    <div id="call-forward-data" style="{% if m.inno[{kz_user_doc_field field1="call_forward" field2="enabled"}] %}
                                           display: block;
                                       {% else %}
                                           display: none;
                                       {% endif %}">
        <div class="form-inline">
             <label>Forward Calls To:</label>
                <input id="ring-number-txt" name="ring-number-txt" type="text" 
                                            ...
                                            value="{{ m.inno[{kz_user_doc_field field1="call_forward" field2="number"}] }}"/>
        </div>
        <div class="form-inline">
            <label>Also Ring My VoIP Phone:</label>
            {% if m.inno[{kz_user_doc_field field1="call_forward" field2="substitute"}] %}
        ...
        <div class="form-inline">
            ...
            {% if m.inno[{kz_user_doc_field field1="call_forward" field2="keep_caller_id"}] %}
    ...
    <div class="form-inline" style="padding-left: 1em; padding-top: 1em;">
        ...
        {% if m.inno[{kz_user_doc_field field1="vm_to_email_enabled"}] %}
    ...
    <div id="vm-to-email-input" class="form-inline" style="padding-left: 3em; padding-top: 1em; padding-bottom: 1em;
        ...
        <input id="vm-to-email-txt" name="vm-to-email-txt" type="text"
                                    ...
                                    value="{{ m.inno[{kz_user_doc_field field1="email"}] }}"/>
    </div>
</form>
</div>
