    ...
    <tbody>
      {% for cred in m.kazoo.kz_cccp_creds_list %}
        {% if cred["pin"] %}
          ...
          {% wire id=cred["id"] action={confirm text=_"Do you really want to delete this Pin?" action={postback postback={postback postback={del_cccp_doc doc_id=cred["id"]} delegate="mod_kazoo"}} %}
        {% endif %}
      {% endfor %}
    </tbody>
