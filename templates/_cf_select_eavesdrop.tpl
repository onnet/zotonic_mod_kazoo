...
<form id="form_cf_select_eavesdrop" method="post" action="postback">
  <div class="form-group">
    <label class="form-control-onnet margin-bottom-xs">Target
      <div class="row">
        <div class="col-sm-6">
            {% wire id="selected_type_target"
                    type="change"
                    action={update target="choice_input_div_target"
                                   template="_group_user_device_selector.tpl"
                                   selector_name="target_selected"}
            %}
            <select id="selected_type_target"
                    name="target_type"
  ...
  <div class="form-group">
    <label class="form-control-onnet margin-bottom-xs">
      ...
      <div class="row">
        <div class="col-sm-6">
          {% wire id="selected_type_approved"
                  type="change"
                  action={update target="choice_input_div_approved"
                                 template="_group_user_device_selector.tpl"
                                 selector_name="approved_selected"
                         }
          %}
          <select id="selected_type_approved"
                  name="approved_type"
                  class="form-control margin-bottom-xs"
                  style="text-align:center;">
            <option value="group"
                    {% if element_data[1]["data"][1]["approved_group_id"] %}
                      selected
                    {% endif %}>Group</option>
            <option value="user"
                    {% if element_data[1]["data"][1]["approved_user_id"] %}
                     selected
                    {% endif %}>User</option>
            <option value="device"
                    {% if element_data[1]["data"][1]["approved_device_id"] %}
                      selected
                    {% endif %}>Device</option>
          </select>
        </div>
...
{% wire id="button_cf_select_eavesdrop"
