...
<table width="100%"><thead><tr><th>
                  ...
                  %}
                <span id="arrows_{{ #topup }}" style="cursor: pointer; padding-left: 0.7em;">
...
<tbody id="top_up_settings" style="display: none;"><tr><td>
               {_ Notify when balance falls below _}
               <input class="input input-xsmall-onnet" type="text" id="balance" name="balance" maxlength="5" size="5" value="{{ notify }}" /> {_ rub _}.
                {% button id="notify_disable_btn" class="btn btn-xs btn-onnet pull-right" text=_"disable alert"
                    action={postback postback={rs_notify_disable_btn uid=uid} delegate="mod_lb"}
                %}
                {% button id="notify_submit_btn" class="btn btn-xs btn-onnet pull-right" text=_"save"
                    action={postback postback={rs_notify_submit_btn uid=uid} delegate="mod_lb" qarg="balance" }
                %}
</td></tr></tbody></table>
{% endwith %}
