...
<form id="form_cf_select_option" method="post" action="postback">
    <div class="form-group">
      <div class="row">
        <div class="col-sm-12">
            {% wire id="menu_key_selector" type="change" action={script script="$('#switch_input').val($('#menu_key_selector').val())" }%}
            <select id="menu_key_selector" name="pre_switch" class="form-control margin-bottom-xs" style="text-align:center;">
              ...
              {% for key in available_keys %}
                  <option value="{{ key }}">{% if key=="match" %}{_ Match CID list _}
                                            {% else %}{_ No match CID list _}
                                            {% endif %}
                  </option>
              {% endfor %}
            </select>
...
<div class="form-group">
  <div class="row">
    <div class="col-sm-12">
      <button id="button_cf_select_option" class="col-xs-12 btn btn-zprimary margin-bottom-xs">{_ Save _}</button>
    </div>
  </div>
</div>
