...
{% wire id="arrows_"++#id type="click"
        ...
        action={postback postback={trigger_innoui_widget arg="numbers_list_widget_opened"}
                         delegate="mod_kazoo"
               }
%}
  <span id="arrows_{{ #id }}" style="cursor: pointer;">
    <i id="arrow_right_{{ #id }}"
       style="{% if m.kazoo[{ui_element_opened element="numbers_list_widget_opened"}] %}
                display: none;
              {% endif %}" 
       class="arrowpad fa fa-arrow-circle-right"></i>
    <i id="arrow_down_{{ #id }}"
       style="{% if not m.kazoo[{ui_element_opened element="numbers_list_widget_opened"}] %}
                display: none;
              {% endif %}" 
  ...
  {% wire id="fix_allocated_numbers"
          action={postback postback={fix_allocated_numbers account_id=account_id}
                           delegate="mod_kazoo"
                 }
  %}
  {% e911_reminder %}
