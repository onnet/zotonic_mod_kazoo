{% with m.inno[{cf_get_element_by_id element_id=element_id}]
    ...
    as 
       ...
       ,devices_short
%}
{% wire id="form_cf_select_ring_group" type="submit" postback="cf_select_ring_group" delegate="inno" %}
<form id="form_cf_select_ring_group" method="post" action="postback">
    ...
    {% for endpoint in element_data[1]["data"][1]["endpoints"] %}
        {% include "_cf_select_ring_group_element.tpl" selected_value=[endpoint] element_type=endpoint["endpoint_type"] account_groups=account_groups users_short=users_short devices_short=devices_short %}
    {% endfor %}
    </div>
    ...
    <div class="form-group">
      <div class="row">
        <div class="col-sm-4">
          ...
          {% wire id="group_selector" type="change" action={ postback postback={cf_ring_group_select element_type="group"}  delegate="inno"}
        ...
        <div class="col-sm-4">
          ...
          {% wire id="user_selector" type="change" action={ postback postback={cf_ring_group_select element_type="user"}  delegate="inno"}
        ...
        <div class="col-sm-4">
          ...
          {% wire id="device_selector" type="change" action={ postback postback={cf_ring_group_select element_type="device"}  delegate="inno"}
    ...
    <div class="form-group">
      <div class="row">
        <div class="col-sm-12">
          {% wire id="cf_select_ring_group" action={script script="$.each($('.sortable'), function() {$('#'+this.id+'_index').val($(this).index()); });"} 
                                            action={script script="$('#"++element_id++"_details').text(($('#group_name').val()))"}
                                            action={submit target="form_cf_select_ring_group"}
          %}
          <button id="cf_select_ring_group" class="col-xs-12 btn btn-zprimary margin-bottom-xs">Commit Ring Group</button>
        </div>
      </div>
    </div>
{% endwith %}
