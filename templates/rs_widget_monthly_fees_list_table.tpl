        ...
        <tr id={{ transaction[1]["id"] }}
          ...
          <td class="td-center1">
            {% if transaction[1]["description"] %}
              ...
              {% if transaction[1]["reason"] == "recurring_prorate" %}
                {_ Prorated MRC _}.
              {% elif transaction[1]["reason"] == "monthly_recurring" %}
                {_ MRC _}.
              {% endif %}
            {% else %}
          ...
          <td class="pri-15 td-right">
            {% if transaction[1]["metadata"][1]["quantity"] %}
              {{ transaction[1]["metadata"][1]["quantity"] }}
            {% else %}
              1
            {% endif %}
          </td>
          ...
          {% if (m.kazoo.kz_current_context_superadmin or m.kazoo.kz_current_context_reseller_status) %}
          <td class="td-center">
            {% if transaction[1]["reason"] == "monthly_recurring" %}
            {% elseif transaction[1]["metadata"][1]["invoice_id"] %}
             <a target="_blank"
          ...
          <td class="td-center">
            {% if transaction[1]["reason"] == "monthly_recurring" %}
            {% else %}
              <i id="delete_fee_{{ transaction[1]["id"] }}" class="fa fa-trash zalarm pointer"></i>
              {% wire id="delete_fee_"++transaction[1]["id"]
