...
<form method="post" action="postback" id="passwordForm">
  <div class="form-group">
   <label class="form-control-onnet margin-bottom-xs">
     {_ Enter an email address for new login name or leave blank to keep an existing one _}
    <div class="row">
      <div class="col-sm-6">
         <input type="text"
                class="form-control margin-bottom-xs mt-1"
                id="username"
                name="username" 
      ...
      <div class="col-sm-6">
        <input type="text"
               ...
               id="email"
               name="email"
               placeholder="{_ Confirm email address _}">
      </div>
    </div>
   </label>
  </div>
    ...
    <div class="row">
      <div class="col-sm-6">
        ...
        <div class="col-sm-6" style="font-size: 0.85em;">
          ...
          <span>{_ 10 Characters long _}</span>
          <br>
        </div>
      </div>
      <div class="col-sm-6">
        <input type="password"
               ...
               name="password2"
               id="password2"
               placeholder="{_ Repeat password _}"
               autocomplete="off">
        <div class="col-sm-6" style="font-size: 0.85em;">
          <span id="pwmatch"
                class="fa fa-remove"
  ...
  <div class="row">
    <div class="col-sm-12">
        {% button id="save_user_creds_btn"
                  class="col-xs-12 btn btn-zprimary mt-1"
                  text=_"Save new user credentials"
                  action={script script="mypwdsubmit()"}
         %}
        {# button id="close_user_creds_window_btn"
                  class="col-xs-12 btn btn-zprimary mt-1 display_none"
                  text=_"Close window"
                  action={dialog_close}
         #}
        {% button id="reset_password_to_random_btn"
                  class="col-xs-12 btn btn-zalarm mt-1 mb-1"
                  text=_"Reset password to random and email to owner"
  ...
  <div id="reset_password_confirmation" class="row display_none">
    <div class="col-sm-12" style="margin-top: 1em; margin-bottom: 1em;">
      {% button id="reset_password_confirmation_btn"
                class="col-xs-12 btn btn-zalarm mt-1 mb-1"
                text=_"Do you really want to reset password?"
                action={postback postback={reset_password username=user_doc["username"]}
                                 delegate="mod_kazoo"
                       }
      %}
      {% button id="cancel_password_confirmation_btn"
                class="col-xs-12 btn btn-zprimary mt-1 mb-1"
                text=_"Back to user's credentials change"
                action={toggle target="passwordForm"}
                action={toggle target="reset_password_confirmation"}
      %}
    </div>
