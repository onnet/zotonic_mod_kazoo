{% with
   ...
   as
     this_cidlistmatch
%}
{% wire id="form_cf_select_cidlistmatch"
        type="submit"
        postback="cf_select_cidlistmatch"
        delegate="mod_kazoo"
%}
<form id="form_cf_select_cidlistmatch" method="post" action="postback">
  <div class="form-group">
    <div class="row">
      <div class="col-sm-12">
        <select id="match_cid_list"
                name="selected"
                class="form-control margin-bottom-xs"
                style="text-align:center;">
          {% for list in m.kazoo.kz_list_account_lists %}
            {% if list[1]["list_type"] == "phone_book" %}
              <option value="{{ list[1]["id"] }}"
                      {% if list[1]["id"] == this_cidlistmatch[1] %}
                        selected
                      {% endif %}>
                {{ list[1]["name"] }}
              </option>
...
<div class="form-group">
  <div class="row">
    <div class="col-sm-12">
      <button id="button_cf_select_cidlistmatch"
...
{% endwith %}
{% wire id="button_cf_select_cidlistmatch"
        action={script script="$('#"++element_id++"_details').text(($('#match_cid_list option:selected').text()))"}
