...
{% wire id="arrows_"++#dtid type="click"
        ...
        action={postback postback={trigger_innoui_widget arg="user_devices_widget_opened"}
                         delegate="mod_kazoo"
               }
%}
  <span id="arrows_{{ #dtid }}" style="cursor: pointer;">
    <i id="arrow_right_{{ #dtid }}"
       style="{% if m.kazoo[{ui_element_opened element="user_devices_widget_opened"}] %}
                display: none;
              {% endif %}"
       class="arrowpad fa fa-arrow-circle-right"></i>
    <i id="arrow_down_{{ #dtid }}"
       style="{% if not m.kazoo[{ui_element_opened element="user_devices_widget_opened"}] %}
                display: none;
              {% endif %}"
...
<div id="user_devices_widget_opened"
     style="{% if not m.kazoo[{ui_element_opened element="user_devices_widget_opened"}] %}
              display: none;
            {% endif %}">
  <div class="text-center p-3">
    {% ilazy class="fa fa-spinner fa-spin fa-3x"
             action={update target="user_devices_widget_opened"
