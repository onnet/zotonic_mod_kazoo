{% with
     m.kazoo[{kz_webhook_info webhook_id=webhook_id}],
     m.kazoo.kz_list_webhooks_ids
   as
     webhook,
     webhooks_ids
%}
    ...
    <div class="row">
      ...
      <div class="col-sm-6">
        ...
        <select id="hook" name="hook" class="form-control margin-bottom-xs" style="text-align:center;">
          ...
          {% if "amocrm"|member:webhooks_ids %}
            <option value="amocrm"
                    {% if "amocrm" == webhook[1]["hook"] %}selected{% endif %}>
              {_ AmoCRM _}
            </option>
          {% endif %}
          {% if "bitrix24"|member:webhooks_ids %}
            <option value="bitrix24"
                    {% if "bitrix24" == webhook[1]["hook"] %}selected{% endif %}>
              {_ Bitrix24 _}
            </option>
          {% endif %}
        </select>
      ...
      <div class="col-sm-4">
        <label class="form-control-onnet margin-bottom-xs">{_ Enabled _}
          <select id="enabled" name="enabled" class="form-control margin-bottom-xs" style="text-align:center;">
            <option value="true"
                    {% if webhook[1]["enabled"] %}selected{% endif %}>
              {_ Enabled _}
            </option>
            <option value="false"
                    {% if not webhook[1]["enabled"] and webhook[1]["id"] %}selected{% endif %}>
              {_ Disabled _}
            </option>
          </select>
        </label>
      ...
      <div class="col-sm-12">
        <label class="form-control-onnet margin-bottom-xs">{_ URI _}
          <input type="text"
                 ...
                 id="uri"
                 name="uri"
                 placeholder="{_ The HTTP URI of the third-party webserver (required) _}"
                 value="{{ webhook[1]["uri"] }}">
        </label>
      </div>
    </div>
...
<div class="form-group">
  <div class="row">
    <div class="col-sm-12">
      {% wire id="add_custom_data_field"
              action={insert_bottom target="custom_data_span" template="_custom_data.tpl"}
      %}
      <button id="add_custom_data_field"
...
<div class="form-group">
  <div class="row">
    <div class="col-sm-12">
      ...
      <button id="save_webhook_btn" class="col-xs-12 btn btn-zprimary margin-bottom-xs">
        {_ Save webhook _}
      </button>
    </div>
  </div>
