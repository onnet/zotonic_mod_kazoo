    ...
    case z_context:get_q("faxfile", Context) of
       ...
       %    AttachmentUrl = <<<<"https://">>/binary,(?TO_BIN(z_dispatcher:hostname(Context)))/binary,<<"/getfaxdoc/id/">>/binary,FileName/binary>>,
            ...
            FaxHeader = case z_context:get_q("company_name_checkbox", Context) of
                            "checked" -> kazoo_util:get_account_name(Context);
...
event({postback,{save_field, Args},_,_}, Context) ->
  ...
  %  Postfix = proplists:get_value(postfix, Args, <<>>),
    AccountId = case proplists:get_value(account_id, Args) of
...
event({postback ,{save_field_select, Args},_,_}, Context) ->
    DocId = proplists:get_value(doc_id, Args),
    FieldName = proplists:get_value(field_name, Args),
    Prefix = proplists:get_value(prefix, Args, <<>>),
  %  Postfix = proplists:get_value(postfix, Args, <<>>),
...
event({submit,cf_select_route_to_cid,_,_},Context) ->
    ElementId = z_context:get_q("element_id", Context),
    EndpointTypes = lists:foldl(fun(EndpointType,J) -> case EndpointType of <<>> -> J; _ -> J ++ [EndpointType] end end
                               ,[]
                               ,z_context:get_q_all('endpoint_types',Context)),
    CidTypes = lists:foldl(fun(CidType,J) -> case CidType of <<>> -> J; _ -> J ++ [CidType] end end
                          ,[]
                          ,z_context:get_q_all('cid_types',Context)),
    _ = cf_builder:cf_set_session('current_callflow'
                                 ,z_string:split(ElementId,"-")++["data","timeout"]
                                 ,z_convert:to_binary(z_context:get_q("timeout", Context))
                                 ,Context),
    _ = cf_builder:cf_set_session('current_callflow'
                                 ,z_string:split(ElementId,"-")++["data","endpoint_types"]
                                 ,EndpointTypes
                                 ,Context),
    _ = cf_builder:cf_set_session('current_callflow'
                                 ,z_string:split(ElementId,"-")++["data","cid_types"]
                                 ,CidTypes
                                 ,Context),
    _ = cf_builder:cf_set_session('current_callflow', z_string:split(ElementId,"-")++["data","can_call_self"], true, Context),
    z_render:dialog_close(Context);
...
event({submit,cf_select_acdc_agent,_,_},Context) ->
    ElementId = z_context:get_q("element_id", Context),
    _ = cf_builder:cf_set_session('current_callflow'
                                 ,z_string:split(ElementId,"-")++["data","action"]
                                 ,z_convert:to_binary(z_context:get_q("selected", Context))
                                 ,Context),
    z_render:dialog_close(Context);
...
event({postback,<<"toggle_all_calls_recording">>,_,_}, Context) ->
    ...
    mod_signal:emit({update_admin_portal_general_settings_tpl, ?SIGNAL_FILTER(Context)}, Context),
    Context;
...
event({submit,sendmail_test_notification,_,_}, Context) ->
    _ = kazoo_util:sendmail_test_notification(Context),
...
event({submit,user_media_settings,_,_}, Context) ->
    ...
    z_render:update(<<"mediasettings">>
                   ,z_template:render("admin_portal_user_media_settings.tpl" ,[{user_id,UserId}] ,Context)
                   ,Context);
...
event({postback,{edit_onbill_variables_dialog,[{account_id, AccountId}]},_,_}, Context) ->
    z_render:dialog(?__("Account onbill details",Context)
                   ,"operations_edit_onbill_variables_dialog.tpl"
                   ,[{account_id, AccountId}]
                   ,Context);
